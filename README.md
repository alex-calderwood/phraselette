...
Three wells use it. The **sieve well** is the context well with the constraints applied while it searches: `src/models/gate.js` turns letter constraints, sound constraints (checked as each word closes, with the CMU dictionary loaded in the worker) and the probability window into a per-token mask that the loops apply while selecting candidates (see `generationGate` in `src/core/constraints.js`), and the well reports how much of the vocabulary the first step let through. The **context well** beam-searches continuations of the preceding text (K beams, K/4 groups); its earlier top-K-then-greedy loop remains in `src/models/lm.js` as "fast search", selectable from the well's ⋯ menu. The **thesaurus** beam-searches the chat reply itself (plain sampling is a menu option): four beams in four groups each write four `<entry>` lines in a row, so a beam sees (and avoids repeating) its own earlier entries while the groups keep the beams apart; all sixteen are collected, best beam first, deduplicated. `scripts/lm-test.mjs --beam` runs either on the CPU.
